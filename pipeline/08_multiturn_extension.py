#!/usr/bin/env python3
"""
Multi-turn extension of SCRIPT: conversation-level choreography and
boundary momentum, null-calibrated in the same way as the core metric.

Two corpora (carebench, hope) are multi-turn. The flat `row` index in
span_events.csv equals the raw annotated CSV's row index (01_extract_events
iterates rows in order), so (conversation, turn) is read directly from the
raw CSVs' `Conversation` and `Turn` columns (HF dataset
abadawi/Cognitive_Atrophy_Benchmark, data/annotated_responses/). Alignment
double-checked two independent ways: file-order mapping via spans_long.csv
agrees exactly (719/719 rows), and per-(row,model,label) event counts
correlate at r = 0.9999 (shifted-control r ~ 0.73-0.79).

Terms (per model, per corpus, and pooled):

  C_x  (conversation choreography) = I(L ; TURN) / H(L)
       "labels have fixed turn slots in the conversation"
       null: permute labels across the whole conversation, preserving each
       turn's event count  -> conversation composition and turn-size
       mixture artefacts cancel.

  M_x  (boundary momentum) = I(F ; P) / H(F)
       F = first highlighted behaviour of turn t+1, P = last of turn t
       (the null-calibrated version of STBF's cross-turn C(a,b))
       null: permute the F's across boundaries within each conversation
       -> per-conversation marginals of firsts and lasts preserved, only
       the specific adjacency pairing is destroyed.

Both computed at the 5-group level (primary; boundary tables are ~320
pairs per model x corpus) and at the full 20-code level (check).

Outputs: tables/multiturn_extension.csv, tables/multiturn_boundary_lift.csv
"""
import numpy as np
import pandas as pd

from paths import EVENTS, TAB
