...
def card(x0, y0, w, h, letter, title, inset=0.0):
    ...
    ax = fig.add_axes((x0 + 0.055 + inset, y0 + 0.16, w - 0.075 - inset, h - 0.33))
...
ax = card(xB, Y0, wB, HH, "b", "One estimator, many schemes", inset=0.1)
...
rows = [
    ("Testbed 1, 5 models", [(val.loc[m, "SCRIPT"], MCOL[m]) for m in MODELS], False),
    ("AnnoMI counsellors", [(float(mc.loc["AnnoMI human counsellors", "SCRIPT"]), HUMAN)], True),
    ("News authors (18 codes)", [(float(pp[pp.layer == "human experts"].SCRIPT.iloc[0]), HUMAN)], True),
    ("Data-to-text errors", [(v, "#66a182") for v in pd.read_csv(TAB / "anchor_d2t.csv").SCRIPT], False),
    ("RAGTruth hallucinations", [(v, "#7c9fb0") for v in pd.read_csv(TAB / "anchor_ragtruth.csv").SCRIPT], False),
    ("WMT24 MT errors", [(v, SYSGREY) for v in pd.read_csv(TAB / "anchor_wmt24.csv").SCRIPT], False),
    ("Labels re-dealt (control)", [(control, "#888888")], False),
]
for y, (lab, pts, human) in enumerate(rows):
    ...
    if len(core) > 1:
        ax.plot([min(core), max(core)], [y, y], color="#dfe3e6", lw=7, solid_capstyle="round", zorder=1)
    for v, col in pts:
        outlier = v >= 0.15 and not human and col == SYSGREY
        ax.scatter(v, y, s=70 if human else 42, color="white" if outlier else col, zorder=3,
...
ax.set_ylim(len(rows) - 0.4, -0.7); ax.tick_params(axis="y", length=0)
ax.set_xlim(-0.04, 0.215); ax.set_xticks([0, 0.05, 0.10, 0.15, 0.20])
ax.set_xlabel("SCRIPT (excess over the within-response shuffle)", fontsize=9.5)
ax.spines["left"].set_visible(False)
ax.annotate("one system,\n140 events", (0.2, 5), textcoords="offset points", xytext=(-4, 9), fontsize=7.5, color=MUTE, ha="right")
ax.legend(handles=[Line2D([], [], marker="D", ls="", ms=7, color=HUMAN, label="human speakers"),
                   Line2D([], [], marker="o", ls="", ms=6, color=SYSGREY, label="LLM systems")],
          fontsize=8, frameon=False, loc="lower right", handletextpad=0.4, borderaxespad=0.2)
