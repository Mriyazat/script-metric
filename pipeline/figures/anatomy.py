#!/usr/bin/env python3
"""What the metric sees: nine replies before and after the within-reply shuffle, and the score."""
import numpy as np
import pandas as pd
import matplotlib
...
from pipeline.common.paths import EVENTS, FIG
...
posv = dc.position.to_numpy()
# slot convention (co-located spans form no transition), identical to sm.compute
C0, M0, R0 = sm._metrics(lab, xbin, rid, len(labels), n_bins, pos=posv, ties="exclude")
rng_null = np.random.default_rng(0)
null = np.array([sm._metrics(sm._shuffle_within(lab, rid, rng_null), xbin, rid,
                             len(labels), n_bins, pos=posv, ties="exclude")
                 for _ in range(200)])
...
# panel headers (the caption refers to the panels as left / middle / right)
axA.set_title("observed", fontsize=10, color="#333333", pad=6)
axB.set_title("one within-reply shuffle", fontsize=10, color="#333333", pad=6)
