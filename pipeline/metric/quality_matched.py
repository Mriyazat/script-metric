#!/usr/bin/env python3
"""Are the SCRIPT-vs-quality gaps a sample-size artefact?

The accurate-empathy and unflagged strata are several times smaller than their
counterparts, and SCRIPT is downward-biased at small n. Both strata are re-scored at a
matched response count (20 resamples per side).

    python -m pipeline.metric.quality_matched ["empathy accuracy"|"harmful flag"]

Writes tables/quality_matched.csv."""
import sys

import numpy as np
import pandas as pd

from pipeline.common.paths import EVENTS, DATA_DIR, TAB, MODELS, MODNUM, CORPORA
