#!/usr/bin/env python3
"""Sensitivity checks, one per argument.

    Q1        SCRIPT within expert-rated quality strata (script_vs_quality.csv)
    Q2        bin-count sweep (bin_sweep.csv)
    Q3        label granularity 20 / 13 / 5 (granularity.csv)
    Q4        skip-gram momentum at lags 1-3 and the sample-size curve (skipgram.csv, sample_size_curve.csv)
    Q5A:<R>   per-annotator paired bootstrap, checkpointed per reviewer
    Q5COLLECT random-effects meta-analysis of the annotator tier gaps (tier_gap_meta.csv)"""
import glob
import json
import sys
...
from pipeline.common.paths import CKPT, CORPORA, DATA_DIR, EVENTS, MODELS, MODNUM, TAB
...
# ---------------------------------------------------------------- Q3
    ...
    def slot_predecessor(rid, pos):
        """Predecessor index under the slot convention: the last event at the
        preceding distinct position in the same reply (-1 if none)."""
    ...
    def skip_mi(lab, xbin, prev, gap):
        """I(L; L_prev<gap> | X) / H(L) at group level, where the lag-`gap`
        predecessor is reached by following slot-convention predecessors."""
        ...
        src = idx.copy()
        for _ in range(gap):
            src = np.where(src >= 0, prev[np.maximum(src, 0)], -1)
        ok = src >= 0
        cur, prv, xb = lab[ok], lab[src[ok]], xbin[ok]
        n = len(cur)
        tot = 0.0
        if n == 0:
            return 0.0
    ...
    for m in MODELS:
        ...
        for gap, nm in [(1, "order1"), (2, "skip1"), (3, "skip2")]:
            v = skip_mi(lab, xb, prev, gap)
            null = np.array([skip_mi(sm._shuffle_within(lab, rid, rng), xb, prev, gap)
                             for _ in range(60)])
...
# ---------------------------------------------------------------- Q5
    ...
    for m in MODELS:
        ...
        for u, dd in d.groupby("unit"):
            ...
            pre[m][u] = (dd.label.map(l2i).to_numpy(),
    ...
    for f in sorted(glob.glob(str(CKPT / "jury_ckpt" / "*.npy"))):
        rev = f.split("/")[-1][:-4]
        A = np.load(f)
        # Gap of the pooled ordering: Claude minus the mean of the other four,
        # following the MODELS order (Qwen, Llama, GPT, Claude, Gemini).
        gap = A[:, 3] - (A[:, 0] + A[:, 1] + A[:, 2] + A[:, 4]) / 4
