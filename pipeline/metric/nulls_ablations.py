#!/usr/bin/env python3
"""Alternative nulls and ablations, one per argument.

    NULLS  circular-shift and within-bin permutation nulls (alternative_nulls.csv)
    LEN    span midpoints and duration deciles instead of starts (length_ablation.csv)
    SHUF   stability of the estimate against the number of shuffles (shuffle_count_stability.csv)
    CARD   calibration cards: scheme, alphabet, entropy, null level per system (calibration_cards.csv)"""
import json
import re
import sys
import unicodedata
...
from pipeline.common.paths import CODES, CORPORA, DATA_DIR, EVENTS, MODELS, MODNUM, RAGTRUTH_PATH, TAB
...
def arrays(m):
    ...
    return lab, xb, rid, len(labels), d.position.to_numpy()
...
if STAGE == "NULLS":
    rows = []
    for m in MODELS:
        lab, xb, rid, nl, pos = arrays(m)
        segs = segments(rid)
        # slot convention (co-located spans form no transition), as in sm.compute
        C, M, R = sm._metrics(lab, xb, rid, nl, NB, pos=pos, ties="exclude")

        def null_R(shuffler, S=100):
            vals = []
            for _ in range(S):
                vals.append(sm._metrics(shuffler(lab), xb, rid, nl, NB, pos=pos, ties="exclude"))
