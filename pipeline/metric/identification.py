#!/usr/bin/env python3
"""The identification game: name the generator of k annotated replies from its profile.

Settings: within corpus, held-out corpus, held-out annotator, both new, and a label
permutation null that must sit at chance (0.20).

    python -m pipeline.metric.identification [--draws 200]

Writes tables/identification_curves.csv and a copy in derived/."""
import argparse
...
def build_profile(df: pd.DataFrame) -> dict:
    ...
    # slot convention (scriptmetric.metric.compute default): co-located spans form no transition
    same = sm._transition_mask(rid, df.position.to_numpy(), "exclude")
...
def _per_response_loglik(pool: pd.DataFrame, ref: dict, alpha: float = 0.5):
    ...
    d = pool.sort_values(["response_id", "position"], kind="stable")
    rid = d.response_id.to_numpy()
    lab = np.array([l2i.get(str(x), -1) for x in d.label.to_numpy()])
    posv = d.position.to_numpy()
    bins = np.minimum((posv * nb).astype(int), nb - 1)
    ...
    for r, li, b, x in zip(rid, lab, bins, posv):
        ...
        # slot convention: no transition term inside a co-located slot
        if prev >= 0 and li >= 0 and x != prev_pos:
            val += logT[prev, li]
