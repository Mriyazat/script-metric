#!/usr/bin/env python3
"""The script profile: position and transition tables per model, their distances,
and the profile figures (atlas, transition lift, timelines, map, cards, identity).

Reads out/derived/identification_curves.csv (run metric/identification.py first).
Writes out/derived/js_fingerprint_distance.csv, profile_reading.csv and the figures."""
import numpy as np
import pandas as pd
import matplotlib
...
from pipeline.common.paths import CORPORA, EVENTS, FIG, MODELS, REF, TAB
...
for k, (m, ax) in enumerate(zip(MODELS, axes)):
    ...
    same = ((d.response_id.values[1:] == d.response_id.values[:-1])
            & (d.position.values[1:] != d.position.values[:-1]))   # slot convention
