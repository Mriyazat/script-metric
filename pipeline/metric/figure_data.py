#!/usr/bin/env python3
"""Cache the data behind the explanatory figures as JSON in out/derived/figdata/:
one annotated reply (fig_reply.json), its pooled tables (fig_pooled.json, faithful.json),
one prompt answered by all five models (hero_data.json), and the momentum walkthrough (mom.json)."""
import json
import re
import unicodedata
...
from pipeline.common.paths import CODES, CORPORA, DATA_DIR, EVENTS, MODELS, MODNUM, REF
...
def count_tables(df, labels):
    ...
    d = df.sort_values(["response_id", "position"], kind="stable").reset_index(drop=True)
...
for ev in resp.values():
    for k in range(len(ev) - 1):
        if ev[k + 1][0] == ev[k][0]:      # co-located spans: one slot, no transition
            continue
        if ev[k][2] == "VIN":
            obs[cidx[ev[k + 1][2]]] += 1
