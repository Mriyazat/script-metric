#!/usr/bin/env python3
"""SCRIPT on the benchmark, with an independent re-extraction of the events.

V0 re-derives every event from the raw CSVs with separate code and checks it against
metric/events.py; V1 scores the five models on all corpora; V2 scores each corpus
alone; V3 runs the random-label control and two synthetic endpoints (perfect template,
pure label chains). Writes out/derived/validation_results.csv, script_by_corpus.csv
and out/verify_report.json."""
import json
import re
import unicodedata
...
from pipeline.common.paths import (CODES, CORPORA, DATA_DIR, DERIVED, EVENTS, MODNUM, OUT)
...
# ------------------------------------ V0: agreement with metric/events.py
# events.py and this file extract the span layer with independent code. They
# must land on the same event set; anything else means one of them is wrong.
print("\n-- V0 cross-check against metric/events.py --")
if EVENTS.exists():
    ...
    print(f"   events.py: {len(E01)} events | this file: {len(E)} events | "
    ...
    print(f"   {EVENTS} not found — run pipeline.metric.events first for V0")
