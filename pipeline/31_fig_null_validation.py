#!/usr/bin/env python3
"""fig_null_validation: the null cancels both artefacts, on known-answer data.

Raw R is inflated by two things that have nothing to do with structure:

  composition        one behaviour dominating the mix makes the next span easy
                     to guess from the mix alone
  small-sample bias  sparse count tables carry accidental mutual information

Both sweeps below generate corpora whose *true* R is 0 by construction: each
event's label is drawn i.i.d. given the response, independent of position and
of the previous label. So every non-zero reading is pure artefact, and the
question is only whether the within-response permutation null removes it.

  (a) composition sweep     dominant-label fraction from uniform to 0.90, N fixed
  (b) small-sample sweep     N from 320 responses down to 10, labels uniform

Raw R climbs in both. SCRIPT should sit flat on zero in both.

    python 31_fig_null_validation.py

Writes figures/fig_null_validation.{png,pdf} and tables/null_validation.csv.
"""
import numpy as np
import pandas as pd
import matplotlib
...
def main() -> None:
    ...
    ax.set_title(f"(a) Composition sweep  —  N fixed, true $R=0$", fontsize=11)
    ...
    ax.set_ylabel("structural share")
    ax.set_title("(b) Small-sample-bias sweep  —  uniform labels, true $R=0$",
                 fontsize=11)
    ax.legend(frameon=False, fontsize=9, loc="upper right")

    fig.suptitle("Null calibration removes composition and small-sample "
                 "artefacts from raw $R$", fontsize=12.5, y=1.02)
    fig.tight_layout()
