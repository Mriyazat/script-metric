...
The clinician span layer only covers the model replies, so it cannot place a
human on the same axis. This stage builds the missing anchor with a
deliberately dumb, fully transparent rule annotator — a sentence-level regex
that emits three labels (questions / empathy / advice) — and runs it over the
five model replies *and* the human therapist's reply to the identical prompts.

Because the same instrument reads both sides, its bias cancels in the
comparison: what remains is arrangement. The cost is resolution — three
surface labels, not the clinician's twenty codes — so the resulting numbers
are comparable only *within* this table, never against the 20-code scores.

    python 12_therapist_baseline.py

Writes tables/therapist_baseline.csv and tables/therapist_js_distance.csv,
plus a copy under derived/ for the ruler figures.
"""
import re
import unicodedata
...
from paths import (CORPORA, DATA_DIR, DERIVED, MODELS, TAB, THERAPIST_COL)
