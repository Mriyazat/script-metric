#!/usr/bin/env python3
"""Contribution 3, part 2 — what the sequential test is actually for.

The paper proposes four uses for SCRIPT: an optimisation target, a regression
test across model versions, a deployment threshold to audit against, and a
provenance audit. Three of those four are streaming decisions, and a
fixed-sample z cannot make them. This stage builds each one.

  A1  CALIBRATION   the betting edge is in nats/event, not on the SCRIPT scale.
                    Fit the map so a ceiling stated as "SCRIPT <= 0.05" can be
                    turned into an `eps` the sequential test can use.
  A2  THRESHOLD     the deployment audit: does this system exceed the ceiling?
                    Composite null H0: delta <= eps, still anytime-valid.
  A3  A/B           the regression test: is A more scripted than B, decided
                    sequentially on paired replies.
  A4  TRANSFER      the same test, unchanged, on the three external span
                    corpora and on AnnoMI — it must decline where the
                    published score already says there is nothing to find.

    python 16_betting_applications.py [A1|A2|A3|A4|ALL]

Writes tables/betting_calibration.csv, betting_threshold.csv,
betting_ab.csv, betting_transfer.csv.
"""
...
def _external_frames():
    ...
    try:
        ...
        for name, frame in [
            ("WMT24 MT error spans",
             anc._load_span_events("mt-eval", anc.MT_LABELS)),
            ("data-to-text error spans",
