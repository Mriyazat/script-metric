#!/usr/bin/env python3
"""fig_betting: the sequential test in one figure.

  (a) wealth trajectories — every model crosses 1/alpha within ~45 replies
      while the null wanders below 1 forever
  (b) sensitivity — how weak a template it still catches, from the dilution
      sweep, with the stopping time on the same axis
  (c) the anytime-valid lower bound on the edge, tightening reply by reply

Reads tables written by 15_betting_validity.py and 16_betting_applications.py.

    python 32_fig_betting.py
"""
import numpy as np
import pandas as pd
import matplotlib
...
from paths import EVENTS, FIG, MODELS, TAB
...
def main() -> None:
    ...
    ax.text(150, 4e15, r"reject: wealth $\geq 1/\alpha = 20$" "\n"
            r"(dotted line)", fontsize=8.5, color="#333333")
    ax.set_yscale("log")
    ax.set_ylim(0.2, 1e17)
    ax.set_xlabel("annotated replies observed")
    ax.set_ylabel("wealth (log scale)")
    ax.set_title("(a) Evidence compounds reply by reply,\nand every look is "
                 "protected", fontsize=10.5)
    ...
    ax.set_title("(b) How weak a template it still catches\n"
                 "(labels of the rest shuffled away)", fontsize=10.5)
    ax.spines[["top", "right"]].set_visible(False)
    ...
    ax.set_title("(c) Anytime-valid lower bound,\nvalid at every t "
                 "simultaneously", fontsize=10.5)
    ax.legend(fontsize=7.6, frameon=False, loc="lower right", ncol=2)
    ax.spines[["top", "right"]].set_visible(False)
    ax.grid(alpha=0.25, lw=0.5)

    fig.suptitle("SCRIPT-Seq — anytime-valid scriptedness testing by betting",
                 fontsize=13, y=1.02)
