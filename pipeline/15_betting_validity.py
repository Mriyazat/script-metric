#!/usr/bin/env python3
"""Contribution 3, part 1 — is the sequential test valid, and how fast is it?

Three things a sequential test has to demonstrate before anyone should use it:

  V1  VALIDITY   under H0 it rejects at most alpha of the time, no matter how
                 long or how often you look. Measured against the tempting
                 alternative — recompute the published z as data arrives and
                 stop when it clears 1.96 — to quantify what peeking costs.
  V2  POWER      when the structure is real, how many annotated replies before
                 it stops? Per model, per corpus, per annotator.
  V3  SCALING    the theory says the expected stopping time goes like 1/delta^2
                 in the edge. Checked against the systems we have.

    python 15_betting_validity.py [V1|V2|V3|ALL] [--streams 200]
...
from paths import CORPORA, EVENTS, MODELS, TAB
...
def naive_peek(d: pd.DataFrame, seed=0, every=20, z_crit=1.96, n_shuffles=30,
    ...
    for t, r in enumerate(order[:max_t], start=1):
        ...
        _, _, R = sm._metrics(lab, xb, rid, len(labels), N_BINS)
