...
from paths import CKPT, CORPORA, DATA_DIR, EVENTS, MODELS, TAB
...
if STAGE.startswith("BOOT"):
    ...
    for b in range(b0, b1):
        ...
        for mi, m in enumerate(MODELS):
            ...
            out[mi] = script_of(lab, xb, ri, len(lab_maps[m]), B_SHUF, rr)
...
def order2_gain(lab, xbin, rid):
    """I(L; L_prev2 | X, L_prev) / H(L) at behaviour-group level."""
    counts = np.bincount(lab, minlength=NG).astype(float)
    p = counts[counts > 0] / counts.sum()
    H_L = -(p * np.log2(p)).sum()
    same2 = (rid[2:] == rid[1:-1]) & (rid[1:-1] == rid[:-2])
    cur, p1, p2, xb = lab[2:][same2], lab[1:-1][same2], lab[:-2][same2], xbin[2:][same2]
    n = len(cur)
    if n == 0:
        return 0.0
    tot = 0.0
...
for i, m in enumerate(MODELS):
    ...
    rid = pd.factorize(d.response_id)[0]
    g2 = order2_gain(lab, xb, rid)
    null = np.array([order2_gain(sm._shuffle_within(lab, rid, rng), xb, rid)
                     for _ in range(60)])
...
# 11_external_anchors.py, which recomputes all three from their sources.
