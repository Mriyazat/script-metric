#!/usr/bin/env python3
"""Download every input from its origin at a pinned revision, then verify it.

  benchmark   Cognitive Atrophy Benchmark, clinician-annotated replies   HuggingFace abadawi/Cognitive_Atrophy_Benchmark
  responses   the same benchmark's full generated-reply corpus           (same repository)
  ragtruth    RAGTruth hallucination spans                               GitHub ParticleMedia/RAGTruth
  spans       data-to-text and WMT24 error spans (Kasner et al.)         GitHub llm-span-annotators/span-annotation
  annomi      AnnoMI motivational-interviewing transcripts               GitHub uccollab/AnnoMI
  mint        empathy-tactic annotations and system outputs (Zhan et al.) GitHub honglizhan/mint-empathy

    python -m pipeline.data.get_data                # everything, pinned
    python -m pipeline.data.get_data benchmark      # one source
    python -m pipeline.data.get_data --verify-only  # integrity checks only

The benchmark CSVs pass a row-alignment test (highlights must be found in the
reply of their own row); the other corpora are checked against the record and
event counts reported in the paper.
"""
...
from pipeline.common.paths import (CODES, CORPORA, DATA_DIR, MINT_DIR, MODELS, MODNUM, RAGTRUTH_PATH, RAW,
                                   RESPONSES_DIR, SPAN_ANNOTATION_DIR)
...
MINT_REPO = "honglizhan/mint-empathy"
MINT_REVISION = "a46182f8f48f1521a6443001c3573eda0d4d2ef2"       # 2026-04-28
...
EXPECT_RESPONSES = {"counselchat": 936, "pair": 276, "carebench": 7016, "hope": 3294}
EXPECT_MINT_SENTENCES = 3164
EXPECT_MINT_CONVERSATIONS = 322
...
def verify_benchmark() -> None:
    for corpus in CORPORA:
        ...
        if not p.exists():
            raise RuntimeError(f"missing {p} — run `get_data benchmark`")
...
def verify_ragtruth() -> None:
    if not RAGTRUTH_PATH.exists():
        raise RuntimeError(f"missing {RAGTRUTH_PATH} — run `get_data ragtruth`")
...
def get_span_annotation(revision: str) -> None:
    ...
    if not marker.exists():
        _download_repo_subtree(SPANS_REPO, revision, SPAN_ANNOTATION_DIR,
                               keep=("annotations/human/", "annotations/model/propaganda/",
                                     "outputs/", "LICENSE", "README.md"))
    verify_span_annotation()


def _count_span_events(kind: str) -> int:
    """Count usable (label, position) events exactly as pipeline.external.anchors does."""
    import glob
    lengths = {}
...
def verify_span_annotation() -> None:
    marker = SPAN_ANNOTATION_DIR / "annotations" / "human" / "mt-eval" / "annotations.jsonl"
    if not marker.exists():
        raise RuntimeError(f"missing {marker} — run `get_data spans`")
...
def verify_annomi() -> None:
    ...
    if not p.exists():
        raise RuntimeError(f"missing {p} — run `get_data annomi`")
    d = pd.read_csv(p)
...
# ----------------------------------------- 5. the benchmark's full reply corpus

def get_responses(revision: str) -> None:
    print(f"[responses] {HF_REPO} @ {revision[:12]}")
    RESPONSES_DIR.mkdir(parents=True, exist_ok=True)
    for corpus in CORPORA:
        fname = f"{corpus}_results.csv"
        dest = RESPONSES_DIR / fname
        if not dest.exists():
            print(f"  fetching {fname}")
            _download_to(f"https://huggingface.co/datasets/{HF_REPO}/resolve/{revision}/"
                         f"data/llm_responses/{corpus}_responses/{fname}", dest)
    verify_responses()


def verify_responses() -> None:
    for corpus in CORPORA:
        p = RESPONSES_DIR / f"{corpus}_results.csv"
        if not p.exists():
            raise RuntimeError(f"missing {p} — run `get_data responses`")
        df = pd.read_csv(p, low_memory=False)
        df.columns = [c.lstrip("\ufeff").strip() for c in df.columns]
        missing = [m for m in MODELS if f"{m} Output" not in df.columns]
        if len(df) != EXPECT_RESPONSES[corpus] or missing:
            raise RuntimeError(f"{corpus} responses changed upstream: {len(df)} rows "
                               f"(expected {EXPECT_RESPONSES[corpus]}), missing {missing}")
        print(f"  {corpus:<12} {len(df):>5} replies per model — OK")


# --------------------------------- 6. MINT: empathy tactics (Zhan et al. 2026)

def get_mint(revision: str) -> None:
    print(f"[mint] {MINT_REPO} @ {revision[:12]}")
    if not (MINT_DIR / "data" / "tagger_annotations" / "all_tagged_sentences.csv").exists():
        _download_repo_subtree(MINT_REPO, revision, MINT_DIR,
                               keep=("data/", "evaluation/outputs/", "evaluation/eval_outputs/",
                                     "LICENSE", "README.md"))
    verify_mint()


def verify_mint() -> None:
    sent = MINT_DIR / "data" / "tagger_annotations" / "all_tagged_sentences.csv"
    conv = MINT_DIR / "data" / "training" / "conversations_322_tagged.json"
    if not (sent.exists() and conv.exists()):
        raise RuntimeError(f"missing MINT data under {MINT_DIR} — run `get_data mint`")
    n_sent = len(pd.read_csv(sent))
    n_conv = len(json.load(open(conv)))
    systems = [d for d in (MINT_DIR / "evaluation" / "outputs").iterdir()
               if (d / "conversations_tagged.json").exists()]
    if n_sent != EXPECT_MINT_SENTENCES or n_conv != EXPECT_MINT_CONVERSATIONS or len(systems) < 23:
        raise RuntimeError(f"MINT release changed upstream: {n_sent} sentences, {n_conv} conversations, "
                           f"{len(systems)} tagged systems")
    print(f"  {n_sent} coded sentences, {n_conv} conversations, {len(systems)} tagged systems — OK")
...
SOURCES = {
    ...
    "responses": (get_responses, verify_responses, HF_REVISION, "main"),
    "mint": (get_mint, verify_mint, MINT_REVISION, "main"),
}
